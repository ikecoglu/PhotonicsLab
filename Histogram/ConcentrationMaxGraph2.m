clear all;close all;clc;load('C:\Users\ibrah\Desktop\Photonics_Workspace\ConcentrationNewCAL.mat')
%% Settings
xstart=1020; xstep=0.1; xstop=1035; ystop=0.1;
BinWidth = 0.1;
...
subplot(2,2,1)
for i =1:size(thp1,1)
    Y(i,:) = interp1(Xn,thp1(i,Str:Stp),xstart:xstep:xstop,'spline');
    y(i)=max(Y(i,:));
    x(i)=X(find(Y(i,:)==y(i),1));
end
clear THP10
h1=histogram(x,edges,'Normalization','probability');
Concentration.THP1(1,:)=middles;
Concentration.THP1(2,:)=h1.Values;
title('THP1 + HT50')
ylim([0 ystop])
xlim([xstart xstop])
clear i x y Y h1
...
Concentration.THP10(2,:)=h2.Values;
ylim([0 ystop])
xlim([xstart xstop])
title('THP10 + HT50')
clear i x y Y h2
...
Concentration.THP25(2,:)=h3.Values;
ylim([0 ystop])
xlim([xstart xstop])
title('THP25 + HT50')
clear i x y Y h3
...
Concentration.THP50(2,:)=h4.Values;
ylim([0 ystop])
xlim([xstart xstop])
title('THP50 + HT50')
clear i x y Y h4
